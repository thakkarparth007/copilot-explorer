...
exports.getPrompt = async function (e, n, g = {}, y = []) {
  ...
  if (E.pathMarker != p.NoMarker) {
    const e = newLineEnded(M_language_marker_constants.getPathMarker(n));
    if (e.length > 0) {
      F = $.append(
        e,
  ...
  function q() {
    j.forEach((e) =>
      $.append(
        ...
        L,
        M_tokenizer.tokenLength(e.snippet),
        e.score
      )
    );
  }
  ...
  if (E.includeSiblingFunctions == h.NoSiblings) U = k.substring(0, I);
    ...
    const {
      ...
      beforeInsertion: t,
      afterInsertion: r,
    } = await M_sibling_function_fetcher.getSiblingFunctions(n);
    $.appendLineForLine(
      t,
    ...
    return $.fulfill(E.maxPromptLength);
  {
    ...
    const p = M_tokenizer.takeFirstTokens(d, c);
    ...
    if (E.suffixMatchCriteria == v.Equal) {
      ...
      if (
        ...
        100 *
          (null ===
            (w = M_edit_distance.findEditDistanceScore(
