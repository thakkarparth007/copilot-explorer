...
const M_esprima_parser_maybe = require("esprima_parser");
const M_esprima_parser_maybe = require("esprima_parser");
var s;
!(function (e) {
  e.Python = "python";
...
async function getLanguage(e) {
  ...
  if (!_.has(t)) {
    const e = await (async function (e) {
      await M_esprima_parser_maybe.init();
      const t = M_path.resolve(
        __dirname,
        "..",
        "dist",
        `tree-sitter-${e}.wasm`
      );
      return M_esprima_parser_maybe.Language.load(t);
    })(t);
    _.set(t, e);
  }
  return _.get(t);
...
async function parseTree(e, t) {
  ...
  const r = new M_esprima_parser_maybe();
  r.setLanguage(n);
  const i = r.parse(t);
  r.delete();
  return i;
