...
exports.WindowedMatcher = class {
  ...
  retrieveAllSnippets(e, t = o.Descending) {
    ...
    const a = this.id() + ":" + e.source;
    const c = null !== (n = s.get(a)) && undefined !== n ? n : [];
    const l = 0 == c.length;
    const u = l ? i.map(this.tokenizer.tokenize, this.tokenizer) : [];
    for (const [e, [t, n]] of this.getWindowsDelineations(i).entries()) {
      ...
      const o = c[e];
      const i = this.similarityScore(o, this.referenceTokens);
      r.push({
        score: i,
        startLine: t,
        endLine: n,
  ...
  findMatches(
    e,
    t = M_prompt_parsing_utils_maybe.SnippetSelectionOption.BestMatch,
    n
  ) {
    if (t == M_prompt_parsing_utils_maybe.SnippetSelectionOption.BestMatch) {
      const t = this.findBestMatch(e);
      return t ? [t] : [];
    }
    return (
      (t == M_prompt_parsing_utils_maybe.SnippetSelectionOption.TopK &&
        this.findTopKMatches(e, n)) ||
      []
    );
  }
  findBestMatch(e) {
