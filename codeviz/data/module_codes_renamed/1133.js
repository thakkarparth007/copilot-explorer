...
function u(e, t) {
  return e !== a.ParsingAndServer ||
    M_getPrompt_main_stuff.isSupportedLanguageId(t)
    ? e
    : a.Server;
}
...
function getConfigDefaultForKey(e) {
  try {
    ...
    throw new Error(
      `Error inspecting config default value ${M_copilot_scheme.CopilotConfigPrefix}.${e}: ${t}`
    );
  }
}
function getConfigDefaultForObjectKey(e, t) {
  try {
    ...
    throw new Error(
      `Error inspecting config default value ${M_copilot_scheme.CopilotConfigPrefix}.${e}.${t}: ${n}`
    );
  }
}
